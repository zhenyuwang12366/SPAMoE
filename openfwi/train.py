...
from utils import SeismicMetrics, EarlyStopping, plot_loss_curve, analyze_fourier_domain, visualize_results  # get_seismic_config, SeismicMetrics, EarlyStopping, plot_loss_curve, etc.
...
# Utilities
# ------------------------------
# 放在 openfwi/train.py 顶部其它 import 之后
...
def get_seismic_config(args):
    ...
    返回:
        config: SeismicMOEConfig 实例，字段覆盖当前脚本用到的全部属性
        runtime_ctx: dict，包含 device/is_logger/world_size/local_rank
    """
    ...
    # —— 数据/任务基本设置 —— #
    cfg.family        = str(getattr(args, "family", "all"))   # 训练目标的类型族：'all' 或具体类型
    cfg.channel_dim   = int(getattr(cfg, "channel_dim", 1))   # 输入通道维度（Processor 用）
    cfg.out_channels  = int(getattr(cfg, "out_channels", 1))  # 速度图输出通道

    # —— 训练超参（与你的脚本中使用到的字段一一对应） —— #
    ...
    # —— 学习率调度（本脚本里只读 milestones/gamma/warmup_iters） —— #
...
def build_zarr_loaders_like_moe(args, config, world_size, local_rank, is_logger):
    ...
    train_ds = ZarrSeismicDataset(
        ...
        expect_input_shape=(5, 1000, 70), to_float32=True,
    )
    ...
    return (train_loader, val_loader, train_sampler, val_sampler,
            input_inverse_transform, output_inverse_transform, data_dict)
...
class GenCriterion(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, gt: torch.Tensor, d_model: Optional[nn.Module] = None) -> Dict[str, torch.Tensor]:
        l1 = self.l1(pred, gt)
        l2 = self.l2(pred, gt)
        ...
        return {"loss_t": loss, "loss": loss.detach(), "l1": l1.detach(), "l2": l2.detach(), "adv": adv.detach()}
...
class WGANGPCriterion(nn.Module):
    ...
    def _grad_penalty(d_model: nn.Module, real: torch.Tensor, fake: torch.Tensor) -> torch.Tensor:
        ...
        grads = torch.autograd.grad(
            outputs=d_inter, inputs=inter, grad_outputs=ones,
    ...
    def forward(self, real: torch.Tensor, fake_detached: torch.Tensor, d_model: nn.Module) -> Dict[str, torch.Tensor]:
        d_real = d_model(real)
...
def run_training(args):
    ...
    (train_loader, val_loader, train_sampler, val_sampler,
     input_inv, output_inv, data_dict) = build_zarr_loaders_like_moe(
        args, config, world_size, local_rank, is_logger
    )
    ...
    # Wrap DDP if needed
    ddp_use = bool(config.distributed.use_distributed)
    if ddp_use:
        model = torch.nn.parallel.DistributedDataParallel(
            model_core.to(device), device_ids=[device.index], output_device=device.index, find_unused_parameters=False
        )
        if use_gan:
            discriminator = torch.nn.parallel.DistributedDataParallel(
                discriminator.to(device), device_ids=[device.index], output_device=device.index, find_unused_parameters=False
            )
    else:
    ...
    if use_gan:
        ...
        optimizer_g = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=lr_g, betas=(0.0, 0.9), weight_decay=args.weight_decay)
        optimizer_d = torch.optim.AdamW([p for p in (discriminator.parameters() if discriminator is not None else []) if p.requires_grad], lr=lr_d, betas=(0.0, 0.9), weight_decay=args.weight_decay)
    else:
        lr = float(config.learning_rate) * (math.sqrt(world_size) if (ddp_use and world_size > 2) else 1.0)
        optimizer_g = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=lr, betas=(0.9, 0.999), weight_decay=getattr(config, 'weight_decay', 1e-2))
        optimizer_d = None
    ...
    if use_gan:
        sched_g = WarmupMultiStepLR(optimizer_g, milestones=lr_milestones, gamma=args.lr_gamma, warmup_iters=warmup_iters, warmup_factor=1e-5)
        sched_d = WarmupMultiStepLR(optimizer_d, milestones=lr_milestones, gamma=args.lr_gamma, warmup_iters=warmup_iters, warmup_factor=1e-5)
    else:
        sched_g = WarmupMultiStepLR(optimizer_g, milestones=lr_milestones, gamma=args.lr_gamma, warmup_iters=warmup_iters, warmup_factor=1e-5)
        sched_d = None

    # Criteria
    g_crit = GenCriterion(args.lambda_g1v, args.lambda_g2v, lambda_adv=(args.lambda_adv if use_gan else 0.0))
    ...
    run_name = "_".join([
        _slugify(gen_key), ("GAN" if use_gan else "PLAIN"), f"lr{getattr(args,'learning_rate',1e-4):g}", f"bs{config.batch_size}", timestamp
    ])
    ...
    if is_logger:
        ...
        with open(results_dir / "config.json", "w", encoding="utf-8") as f:
            json.dump(vars(config) if not hasattr(config, "to_dict") else config.to_dict(), f, indent=2, default=str)
    ...
    if getattr(args, "resume_path", None) and os.path.exists(args.resume_path):
        ...
        if ("optimizer_g" in ckpt) and (optimizer_g is not None):
            try:
                ...
                if is_logger: print(f"[Resume][optG] skip: {e}")
        if use_gan and ("optimizer_d" in ckpt) and (optimizer_d is not None):
            try:
                optimizer_d.load_state_dict(ckpt["optimizer_d"])
            except Exception as e:
                if is_logger: print(f"[Resume][optD] skip: {e}")
    ...
    early_stopper = EarlyStopping(
        ...
        mode="min"
    ) if getattr(config, "early_stop", False) else None
    ...
    try:
        for epoch in range(start_epoch, config.epochs):
            if ddp_use and hasattr(train_loader, 'sampler') and hasattr(train_loader.sampler, 'set_epoch'):
            ...
            running = {"g_loss": 0.0, "l1": 0.0, "l2": 0.0, "adv": 0.0, "d_diff": 0.0, "d_gp": 0.0, "seen": 0}
            ...
            for it, batch in enumerate(pbar):
                ...
                if do_g:
                    ...
                    with torch.amp.autocast(device_type=device.type, enabled=amp_enabled, dtype=torch.bfloat16):
                        ...
                        pack = g_crit(pred.to(torch.float32), y, (discriminator if use_gan else None))
                    pack["loss_t"].backward()
                    if hasattr(torch.nn.utils, "clip_grad_norm_") and getattr(config, "max_norm", 0.0) > 0:
                ...
                if is_logger and tb_writer and (it % max(1, getattr(config, "log_every", 50)) == 0):
            ...
            if is_logger:
                ...
                print(f"[Epoch {epoch}] train_g_loss={g_loss_avg:.6f}  val_loss={val_loss:.6f}  PSNR={val_stats['psnr']:.4f}  SSIM={val_stats['ssim']:.4f}")
                if tb_writer:
                    tb_writer.add_scalar("val/loss", val_loss, (epoch+1)*len(train_loader))
                    tb_writer.add_scalar("val/psnr", val_stats['psnr'], (epoch+1)*len(train_loader))
                    tb_writer.add_scalar("val/ssim", val_stats['ssim'], (epoch+1)*len(train_loader))
            ...
            if improved and is_logger:
                ...
                to_save = {
                    ...
                    "discriminator_state_dict": ((discriminator.module.state_dict() if hasattr(discriminator, "module") else discriminator.state_dict()) if (use_gan and discriminator is not None) else None),
                    "optimizer_g": optimizer_g.state_dict(),
                    "optimizer_d": (optimizer_d.state_dict() if (use_gan and (optimizer_d is not None)) else None),
                    "epoch": epoch,
                    "best_loss": best_val,
                    "data_dict": data_dict,
                }
...
def run_inference(args):
    ...
    for bi, batch in enumerate(_tqdm.tqdm(loader, desc='Infer', disable=not is_logger)):
        ...
        if output_inv := output_inv if callable(output_inv) else None:
            pass  # keep mypy silent
        pred_v   = output_inv(pred) if output_inv else pred
        target_v = output_inv(y) if output_inv else y

        # Limit number of saved samples per batch
        num_samples = min(max_vis, x_v.shape[0])
        visualize_results(
            x_v.to(torch.float32), target_v.to(torch.float32), pred_v.to(torch.float32),
            save_dir=str(vis_dir), max_samples=num_samples,
            tb_writer=None, wandb_run=None, global_step=bi,
        )
        analyze_fourier_domain(
            x_v.to(torch.float32), target_v.to(torch.float32), pred_v.to(torch.float32),
            save_dir=str(fft_dir), max_samples=num_samples,
            tb_writer=None, wandb_run=None, global_step=bi,
        )
...
def build_argparser_and_parse():
    import argparse
    p = argparse.ArgumentParser("Plain Generator Training/Inference on moe‑like pipeline with optional WGAN‑GP")
    ...
    # Generator
    p.add_argument("--generator", type=str, default="InversionNet", choices=["InversionNet", "UPFWI"]) 
    p.add_argument("--up_mode", type=str, default=None)
    p.add_argument("--sample_spatial", type=float, default=1.0)

    # Plain training hyperparams (kept from moe facade)
    ...
    p.add_argument("--infer_split", type=str, default="val", choices=["train", "val"]) 
